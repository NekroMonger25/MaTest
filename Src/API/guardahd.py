...
async def search(clean_id,client):
    ...
    response = await client.get(ForwardProxy + f"https://guardaproxy.steveceltis.workers.dev/set-movie-a/{clean_id}", allow_redirects=True, impersonate = "chrome124", headers = headers, proxies = proxies)
